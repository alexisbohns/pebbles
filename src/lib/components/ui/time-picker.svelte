	...
	<PopoverContent class="w-[260px] space-y-4 p-4" align="start">
		<div class="grid gap-4">
			<div class="grid gap-2 text-center">
				...
				<div class="flex items-center justify-center gap-2">
					<Button type="button" variant="outline" size="icon" onclick={decrementHour}>-</Button>
					<span class="w-12 text-lg font-semibold tabular-nums">
						{pad(internalHour)}
					</span>
					<Button type="button" variant="outline" size="icon" onclick={incrementHour}>+</Button>
				</div>
			</div>
			...
			<div class="grid gap-2 text-center">
				...
				<div class="flex items-center justify-center gap-2">
					<Button type="button" variant="outline" size="icon" onclick={decrementMinute}>-</Button>
					<span class="w-12 text-lg font-semibold tabular-nums">
						{pad(internalMinute)}
					</span>
					<Button type="button" variant="outline" size="icon" onclick={incrementMinute}>+</Button>
				</div>
			</div>
			...
			<div class="grid gap-2">
				...
				<Input type="time" step={stepMinutes * 60} {value} oninput={handleTimeInput} />
			</div>
		</div>
		...
		<div class="flex items-center justify-between gap-2">
			...
			<div class="flex gap-2">
				<Button type="button" variant="outline" size="sm" onclick={applyCurrentTime}>Now</Button>
				<PopoverClose>
					{#snippet child({ props })}
						<Button {...props} type="button" size="sm">Done</Button>
					{/snippet}
				</PopoverClose>
			</div>
		</div>
